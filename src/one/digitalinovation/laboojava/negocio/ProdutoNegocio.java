...
public class ProdutoNegocio {
    ...
    public void excluir(String codigo) {
        ...
        //criamos a variavel que vai receber o codigo de exclusao
        int produtoASerExcluido = -1;

        //Criamos um for para percorrer a lista de dados buscados no db
        for(int i = 0; i < bancoDados.getProdutos().length;i++){
            //com a variavel i(ndex) preenchida continuamos percorrendo ate q n sobre mais nada
            Produto produto = bancoDados.getProdutos()[i];
            //Aqui confirmamos se realmente é um codigo
            if(produto.getCodigo().equals(codigo)){
                produtoASerExcluido =i;
                break;
            }
        }
        /*
        * Com todas as informações necessarias ja resgatadas do db e validadas pelo equals
        * fazmos a real remoção do produto.
        * */
        if(produtoASerExcluido != -1){
            bancoDados.removerProduto(produtoASerExcluido);
